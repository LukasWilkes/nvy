...
import { mapActions } from "pinia"
  ...
  export function ViewPortZoom() {
    ...
    const selectedZoomOption = zoomOptions?.find(zOption => zOption.value === settings?.scale)
            ...
            <div className=" flex items-center">
                ...
                <Popover>
                    <PopoverTrigger className="w-24 rounded-none justify-between" asChild>
                        <Button variant="secondary">{(settings?.scale * 100)?.toFixed(0)}%<ChevronDown /></Button>
                    </PopoverTrigger>
                    <PopoverContent>
                        <div className="flex flex-col">
                            {zoomOptions?.map(zOption => (
                                <Button variant={'ghost'} className="justify-between" key={zOption.name} onClick={() => {
